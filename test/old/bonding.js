...
// describe('XYZDAOBondingCalculator', 
//     () => {
...
//         XYZDAOTreasury = await ethers.getContractFactory('XYZDAOTreasury');
//         XYZDAOTreasury = await XYZDAOTreasury.deploy();

//         XYZDAOBondingCalculator = await ethers.getContractFactory('XYZDAOBondingCalculator');
//         XYZDAOBondingCalculator = await XYZDAOBondingCalculator.deploy();
...
//         await XYZDAOTreasury.setOlmypusTokenAddress(tt1.address);
//         await XYZDAOTreasury.setBondingContractAddress(olyUniV2CompatiableLPTokenBonding.address);
//         await XYZDAOTreasury.setBondingCalcContract(XYZDAOBondingCalculator.address);
//     })
...
//           async () => {
//             await expect(olyUniV2CompatiableLPTokenBonding.connect(depositor).initialize(tt1.address,XYZDAOTreasury.address))
...
//             async () => {
//                 await olyUniV2CompatiableLPTokenBonding.initialize(tt1.address,XYZDAOTreasury.address);
...
//             async () => {
//                 await olyUniV2CompatiableLPTokenBonding.initialize(tt1.address,XYZDAOTreasury.address);
//                 await expect(olyUniV2CompatiableLPTokenBonding.initialize(tt1.address,XYZDAOTreasury.address))
...
//                         pair.address,bondSaclingFactor_,bondingPeriodInBlocks_,XYZDAOBondingCalculator.address))
//                     .to.be.revertedWith( "Ownable: caller is not the owner" );
//                 }
//             );
...
//                     expect(await olyUniV2CompatiableLPTokenBonding.principleValuationCalculator(pair.address)).to.be.equal(XYZDAOBondingCalculator.address);
...
//                     await olyUniV2CompatiableLPTokenBonding.initialize(pair.address,XYZDAOTreasury.address);
//                     await olyUniV2CompatiableLPTokenBonding.addBondTerm(
//                         pair.address,bondSaclingFactor_,bondingPeriodInBlocks_,XYZDAOBondingCalculator.address
//                     )
...
//                     const interest  = Number(BigInt(await XYZDAOBondingCalculator.principleValuation(pair.address,amountToDeposit_)));
...
//                     const treasuryContractBalBeforeUserRedeem = BigInt(await pair.balanceOf(XYZDAOTreasury.address)).toString();
...
//                     const treasuryContractBalAfterUserRedeem = BigInt(await pair.balanceOf(XYZDAOTreasury.address)).toString();
