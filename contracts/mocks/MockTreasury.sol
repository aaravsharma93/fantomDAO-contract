...
contract MockXYZDAOTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sXYZ;
  uint256 public sXYZQueue; // Delays change to sXYZ address
  ...
  constructor(
    ...
    // isLiquidityToken[ _XYZDAI ] = true;
    // liquidityTokens.push( _XYZDAI );
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sXYZ).balanceOf(msg.sender); // Can only borrow against sXYZ held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithXYZ(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(XYZ).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SXYZ) {
      // 9
