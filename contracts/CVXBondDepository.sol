...
contract XYZDAOCVXBondDepository is Ownable {
    ...
    address public immutable treasury; // mints XYZ when receives principal
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( XYZ ).safeTransfer( _recipient, _amount ); // send payout
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principal or XYZ) to the DAO
     *  @return bool
     */
