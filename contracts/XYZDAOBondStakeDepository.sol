...
contract XYZDAOBondStakeDepository is Ownable {
    ...
    address public immutable XYZ; // intermediate reward token from treasury
    address public immutable sXYZ; // token given as payment for bond
    ...
    struct Bond {
        uint gonsPayout; // sXYZ gons remaining to be paid
        uint XYZPayout; //XYZ amount at the moment of bond
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISXYZ(sXYZ).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sXYZ ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sXYZ balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISXYZ(sXYZ).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = ISXYZ(sXYZ).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
